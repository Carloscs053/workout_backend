    ...
    try {
        ...
        if (filterParams.mode) {
            workouts = workouts.filter( workout => {
                return workout.mode.toLowerCase().includes(filterParams.mode.toLowerCase())
        ...
        if (filterParams.sort) {
            if (filterParams.sort === 'asc'.toLowerCase()) {
                workouts.sort((a, b) => new Date(a.createdAt).getTime > new Date(b.createdAt).getTime)
            }
...
const createNewWorkout = (workoutToInsert) => {
    const workout = DB.workouts.findIndex((workout) => workout.id === workoutToInsert.id) > -1
    if (workout) {
        throw {
            status: 400,
...
const updateOneWorkout = (workoutId, changes) => {
    try {
        ...
        const indexForUpdate = DB.workouts.findIndex((workout) => workout.id === workoutId) > -1
        if (indexForUpdate === -1) {
            throw {
                status: 400,
